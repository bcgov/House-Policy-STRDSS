/* Create all Sprint 7 DB Objects STR DSS */
...
CREATE OR REPLACE VIEW dss_rental_listing_vw AS SELECT drl.rental_listing_id,
    ...
    ( SELECT sum(drl2.nights_booked_qty) AS sum
           FROM dss_rental_listing drl2
          WHERE ((drl2.offering_organization_id = drl.offering_organization_id) AND ((drl2.platform_listing_no)::text = (drl.platform_listing_no)::text))) AS nights_booked_ytd_qty,
    ( SELECT sum(drl2.separate_reservations_qty) AS sum
           FROM dss_rental_listing drl2
          WHERE ((drl2.offering_organization_id = drl.offering_organization_id) AND ((drl2.platform_listing_no)::text = (drl.platform_listing_no)::text))) AS separate_reservations_ytd_qty,
