 ...
 *
 * Scope: email-sourced action types only (NonComplianceNotice, TakedownRequest, PlatformTakedown).
...
WHERE em.concerned_with_rental_listing_id IS NOT NULL
  AND em.email_message_type IN ('Notice of Takedown', 'Takedown Request', 'Completed Takedown')
  AND NOT EXISTS (
	SELECT 1
