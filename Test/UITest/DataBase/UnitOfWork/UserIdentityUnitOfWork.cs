...
{
    public class UserIdentityUnitOfWork : IDisposable, IUnitOfWork
    {
        DbContext _context;
        bool _disposed;
        ...
        public UserIdentityUnitOfWork(DbContext context)
        {
            if (context != null)
            {
                _context = context;
        ...
        public DssUserIdentity? GetIdentity(string email)
        {
            ...
            var identity = _DssUserIdentityRepository._DbSet.Where(p => p.EmailAddressDsc == email).FirstOrDefault<DssUserIdentity>();
