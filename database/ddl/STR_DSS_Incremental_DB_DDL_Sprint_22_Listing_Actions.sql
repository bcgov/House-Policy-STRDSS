/* STR DSS Sprint 22 - Listing action tracking (incremental DDL)
 * Run as strdssdev: psql -U strdssdev -d strdssdev -f STR_DSS_Incremental_DB_DDL_Sprint_22_Listing_Actions.sql
 */
...
CREATE UNIQUE INDEX dss_rental_listing_action_u1
	ON dss_rental_listing_action ( source_email_message_id )
	WHERE source_email_message_id IS NOT NULL;

CREATE INDEX dss_rental_listing_action_i1
	ON dss_rental_listing_action ( rental_listing_id, action_dtm DESC, rental_listing_action_id DESC );

ALTER TABLE dss_rental_listing_action ADD CONSTRAINT dss_rental_listing_action_fk_listing
	FOREIGN KEY ( rental_listing_id ) REFERENCES dss_rental_listing( rental_listing_id );

ALTER TABLE dss_rental_listing_action ADD CONSTRAINT dss_rental_listing_action_fk_type
	FOREIGN KEY ( listing_action_type ) REFERENCES dss_listing_action_type( listing_action_type );

ALTER TABLE dss_rental_listing_action ADD CONSTRAINT dss_rental_listing_action_fk_user
	FOREIGN KEY ( initiating_user_identity_id ) REFERENCES dss_user_identity( user_identity_id );

ALTER TABLE dss_rental_listing_action ADD CONSTRAINT dss_rental_listing_action_fk_email
	FOREIGN KEY ( source_email_message_id ) REFERENCES dss_email_message( email_message_id );

COMMENT ON TABLE dss_listing_action_type IS
	'A type of action that can be recorded against a rental listing';

COMMENT ON COLUMN dss_listing_action_type.listing_action_type IS
	'System-consistent code for the listing action type';

COMMENT ON COLUMN dss_listing_action_type.listing_action_type_nm IS
	'Business term for the listing action type';

COMMENT ON COLUMN dss_listing_action_type.listing_action_sort_no IS
	'Relative order in which the business prefers to see the action type listed';

COMMENT ON COLUMN dss_listing_action_type.action_source_type IS
	'Indicates who or what typically originates this action type (User, Platform, System)';

COMMENT ON TABLE dss_rental_listing_action IS
	'A user-facing action recorded against a rental listing';

COMMENT ON COLUMN dss_rental_listing_action.rental_listing_action_id IS
	'Unique generated key';

COMMENT ON COLUMN dss_rental_listing_action.rental_listing_id IS
	'Foreign key to the master rental listing';

COMMENT ON COLUMN dss_rental_listing_action.listing_action_type IS
	'Foreign key to the listing action type';

COMMENT ON COLUMN dss_rental_listing_action.action_dtm IS
	'Timestamp when the action occurred';

COMMENT ON COLUMN dss_rental_listing_action.action_short_nm IS
	'Short-form display label for listing and aggregate pages';

COMMENT ON COLUMN dss_rental_listing_action.action_long_nm IS
	'Long-form display label for detail page and data download';

COMMENT ON COLUMN dss_rental_listing_action.takedown_reason IS
	'Platform takedown reason when listing_action_type is PlatformTakedown (e.g. LG Request, Invalid Registration)';

COMMENT ON COLUMN dss_rental_listing_action.initiating_user_identity_id IS
	'User who initiated the action, when applicable';

COMMENT ON COLUMN dss_rental_listing_action.source_email_message_id IS
	'Optional link to the originating email message when an email was sent';

COMMENT ON COLUMN dss_rental_listing_action.upd_dtm IS
