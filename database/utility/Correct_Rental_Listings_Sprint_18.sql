...
USING (
	SELECT
		...
		CASE WHEN dbl.business_licence_id IS NULL
			THEN regexp_replace(regexp_replace(UPPER(drl.business_licence_no), '[^A-Z0-9]+', '', 'g'), '^0+', '')
			ELSE regexp_replace(regexp_replace(UPPER(dbl.business_licence_no), '[^A-Z0-9]+', '', 'g'), '^0+', '')
...
USING (
	SELECT
		drl.rental_listing_id,
		regexp_replace(regexp_replace(UPPER(drlc.full_nm), '[^A-Z0-9]+', '', 'g'), '^0+', '') AS effective_host_nm
	FROM dss_rental_listing drl
