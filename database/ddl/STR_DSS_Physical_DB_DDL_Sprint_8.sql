...
CREATE  TABLE dss_upload_line ( 
	...
	CONSTRAINT dss_upload_line_uk UNIQUE ( including_upload_delivery_id, source_record_no ) 
 );
...
CREATE OR REPLACE VIEW dss_rental_listing_vw AS SELECT drl.rental_listing_id,
    ...
    ( SELECT sum(drl2.nights_booked_qty) AS sum
           FROM dss_rental_listing drl2
          WHERE ((drl2.derived_from_rental_listing_id IS NULL) AND (drl2.offering_organization_id = drl.offering_organization_id) AND ((drl2.platform_listing_no)::text = (drl.platform_listing_no)::text))) AS nights_booked_ytd_qty,
    ( SELECT sum(drl2.separate_reservations_qty) AS sum
           FROM dss_rental_listing drl2
          WHERE ((drl2.derived_from_rental_listing_id IS NULL) AND (drl2.offering_organization_id = drl.offering_organization_id) AND ((drl2.platform_listing_no)::text = (drl.platform_listing_no)::text))) AS separate_reservations_ytd_qty,
